...
public class GoldCoinRMBblockProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((world.getBlockState(BlockPos.containing(x, y + 1, z))).getBlock() == Blocks.AIR || (world.getBlockState(BlockPos.containing(x, y + 1, z))).getBlock() == Blocks.CAVE_AIR) {
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(CoinflipModItems.GOLD_COIN.get());
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
			}
			world.setBlock(BlockPos.containing(x, y + 1, z), CoinflipModBlocks.GOLD_COIN_BLOCK.get().defaultBlockState(), 3);
		}
	}
}
