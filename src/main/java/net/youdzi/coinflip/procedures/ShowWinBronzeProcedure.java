...
public class ShowWinBronzeProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		if (world.isClientSide()) {
		    ...
		    if (player != null && player.getCapability(CoinflipModVariables.PLAYER_VARIABLES_CAPABILITY, null)
		        .orElse(new CoinflipModVariables.PlayerVariables()).showcoin == 2) {
		        
		        Minecraft.getInstance().gameRenderer.displayItemActivation(new ItemStack(CoinflipModItems.REVERTED_BRONZE_COIN.get()));
		    }
		}
	}
}
